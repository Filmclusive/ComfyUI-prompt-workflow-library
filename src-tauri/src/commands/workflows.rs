...
use crate::{
    commands::comfyui::spawn_comfyui_app,
    ...
    match scope {
        WorkflowScope::Global => {
            let app = app.ok_or_else(|| {
                AppError::InvalidInput("App handle required for global workflows".to_string())
            })?;
            let dir = app
                .path_resolver()
                .app_data_dir()
        ...
        WorkflowScope::Project => {
            let project_dir = project_dir.ok_or_else(|| {
                AppError::InvalidInput("projectDir is required for project workflows".to_string())
            })?;
...
fn parse_workflow_id(dir_name: &str) -> AppResult<Uuid> {
    ...
    if !dir_name.starts_with(prefix) {
        return Err(AppError::InvalidInput(format!(
            "Invalid workflow dir: {dir_name}"
        )));
    }
    let id = &dir_name[prefix.len()..];
    Ok(Uuid::parse_str(id)
        .map_err(|_| AppError::InvalidInput("Invalid workflow id".to_string()))?)
}
...
fn import_workflow_impl(
    ...
    copy_file_atomic(
        &PathBuf::from(workflow_json_src),
        &workflow_json_path(&wdir),
    )?;
...
#[tauri::command(rename_all = "snake_case")]
pub fn get_workflow_json(
    app: tauri::AppHandle,
    scope: WorkflowScope,
    project_dir: Option<String>,
    workflow_id: Uuid,
) -> Result<String, String> {
    get_workflow_json_impl(Some(&app), scope, project_dir.as_deref(), workflow_id)
        .map_err(|e| e.to_string())
}

fn get_workflow_json_impl(
    app: Option<&tauri::AppHandle>,
    scope: WorkflowScope,
    project_dir: Option<&str>,
    workflow_id: Uuid,
    ...
    let json: serde_json::Value = read_json(&src)?;
    Ok(serde_json::to_string_pretty(&json)?)
}

#[tauri::command(rename_all = "snake_case")]
pub fn open_workflow_in_comfyui(
    app: tauri::AppHandle,
    scope: WorkflowScope,
    project_dir: Option<String>,
    workflow_id: Uuid,
) -> Result<(), String> {
    open_workflow_in_comfyui_impl(&app, scope, project_dir.as_deref(), workflow_id)
        .map_err(|e| e.to_string())
}

fn open_workflow_in_comfyui_impl(
    app: &tauri::AppHandle,
    scope: WorkflowScope,
    project_dir: Option<&str>,
    workflow_id: Uuid,
    ...
    let app_path = settings
        .comfyui
        .app_path
        .ok_or_else(|| AppError::InvalidInput("ComfyUI application is not set".to_string()))?;

    // Ensure the workflow exists so the UI can confidently guide the user to paste/import it.
    let root = workflows_root_dir(Some(app), &scope, project_dir)?;
    let wdir = workflow_dir(&root, workflow_id);
    let src = workflow_json_path(&wdir);
    if !src.exists() {
        return Err(AppError::NotFound("Workflow JSON not found".to_string()));
    ...
    // Best-effort: just open ComfyUI itself.
    // Loading workflows differs across ComfyUI builds, so the UI guides the user to paste/import.
    spawn_comfyui_app(&app_path, None)?;
    Ok(())
}

#[tauri::command(rename_all = "snake_case")]
pub fn reveal_workflow_json(
    app: tauri::AppHandle,
    scope: WorkflowScope,
    project_dir: Option<String>,
    workflow_id: Uuid,
) -> Result<(), String> {
    reveal_workflow_json_impl(&app, scope, project_dir.as_deref(), workflow_id)
        .map_err(|e| e.to_string())
}

fn reveal_workflow_json_impl(
    app: &tauri::AppHandle,
    scope: WorkflowScope,
    project_dir: Option<&str>,
    workflow_id: Uuid,
    ...
    {
        let arg = format!("/select,{}", src.to_string_lossy());
    ...
    #[cfg(not(any(target_os = "macos", target_os = "windows")))]
    {
        let dir = src
            .parent()
            .ok_or_else(|| AppError::InvalidInput("Invalid workflow path".to_string()))?;
...
pub fn read_workflow_meta(
    ...
    read_workflow_meta_impl(Some(&app), scope, project_dir.as_deref(), workflow_id)
        .map_err(|e| e.to_string())
}
