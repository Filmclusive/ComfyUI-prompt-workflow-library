...
use crate::{
  ...
  paths::{
    ensure_dir, scene_dir, scene_json_path, scene_shots_dir, shot_dir, shot_dir_name, shot_json_path,
    shot_negative_path, shot_positive_path,
  },
...
fn list_shots_impl(project_dir: &str, scene_dir_name: &str, scene_id: Uuid) -> AppResult<Vec<Shot>> {
...
fn create_shot_impl(project_dir: &str, scene_id: Uuid) -> AppResult<Shot> {
  ...
  let scene_dir_name = scene_dir_name.ok_or_else(|| AppError::NotFound("Scene not found".to_string()))?;
  let sdir = scene_dir(&project_dir, &scene_dir_name);
...
fn update_shot_fields_impl(project_dir: &str, scene_id: Uuid, shot_id: Uuid, shot: &Shot) -> AppResult<()> {
  let project_dir = PathBuf::from(project_dir);
  let scenes_root = project_dir.join("scenes");
  for scene_entry in std::fs::read_dir(&scenes_root)? {
    let scene_entry = scene_entry?;
