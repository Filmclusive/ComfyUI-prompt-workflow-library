...
fn create_revision_impl(project_dir: &str, scene_id: Uuid, shot_id: Uuid, message: Option<String>) -> AppResult<()> {
...
fn list_revisions_impl(project_dir: &str, scene_id: Uuid, shot_id: Uuid) -> AppResult<Vec<RevisionSummary>> {
  let project_dir = PathBuf::from(project_dir);
  let shot_dir = find_shot_dir(&project_dir, scene_id, shot_id)?;
  let history_root = shot_history_dir(&shot_dir);
...
pub fn restore_revision(
  ...
  restore_revision_impl(&project_dir, scene_id, shot_id, &revision_dir_name).map_err(|e| e.to_string())
}

fn restore_revision_impl(project_dir: &str, scene_id: Uuid, shot_id: Uuid, revision_dir_name: &str) -> AppResult<()> {
  let project_dir = PathBuf::from(project_dir);
  let shot_dir = find_shot_dir(&project_dir, scene_id, shot_id)?;
...
pub fn diff_revision(
  ...
  diff_revision_impl(&project_dir, scene_id, shot_id, &revision_dir_name).map_err(|e| e.to_string())
}

fn diff_revision_impl(project_dir: &str, scene_id: Uuid, shot_id: Uuid, revision_dir_name: &str) -> AppResult<DiffResult> {
  let project_dir = PathBuf::from(project_dir);
  let shot_dir = find_shot_dir(&project_dir, scene_id, shot_id)?;
  let rev_dir = shot_history_dir(&shot_dir).join(revision_dir_name);
  if !rev_dir.exists() {
  ...
  let pos_diff = TextDiff::from_lines(&rev_pos, &cur_pos).unified_diff().to_string();
  let neg_diff = TextDiff::from_lines(&rev_neg, &cur_neg).unified_diff().to_string();
